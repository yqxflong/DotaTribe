...
bool BoutiqueView::onTick(){

	return true;
}
bool BoutiqueView::onUpdata()
{
...
void BoutiqueView::EventWindowCallBack(int iEventType, std::list<std::string>& paramList)
{
	if(iEventType == _TYPED_EVENT_UPDATE_MARKETPWINDOW_)
	{
		onUpdata();
	}
...
void BoutiqueView::_initLayout(){
	_initData();
	_setMoneyText();
	_showMoneyText();
	_initCell();
	m_isShowArrow = SHOPINFODATAHANDLER->m_iShowArrow;
	SHOPINFODATAHANDLER->m_iShowArrow = false;
}

void BoutiqueView::_initData()
{
	UIVIEWDATAMANAGER->AutoCreateNodeByTable(UIViewDataManager::kSHOPVIEW,this);
}

void BoutiqueView::_initCell()
{
	CCNode* node=(CCNode*)this->getChildByTag(udTABLEVIEWCELL);
	m_szCell = node->getContentSize();
}

void BoutiqueView::_setTableView(){
...
CCSize BoutiqueView::cellSizeForTable(CCTableView *table){
	return m_szCell;
};

CCTableViewCell* BoutiqueView::tableCellAtIndex(CCTableView *table, unsigned int idx){
	CCTableViewCell *cell = table->dequeueCell();
	if (!cell) {
		cell = new CCTableViewCell();
		cell->autorelease();
		_createTableCell(cell);	
		if (m_isShowArrow && !m_isAddArrow)
		{
			m_isAddArrow = true;
		}
	}
...
unsigned int BoutiqueView::numberOfCellsInTableView(CCTableView *table){
	
	return numberOfCells;//计算有多少个cell
};


void BoutiqueView::_createTableCell(cocos2d::extension::CCTableViewCell * cell){
	UIVIEWDATAMANAGER->AutoCreateNodeByTable(UIViewDataManager::kSHOPVIEWCELL,cell);
} 


void BoutiqueView::_setAllCurrencyVisible(cocos2d::extension::CCTableViewCell* cell,bool isible,int blockIdx)
{
	...
	if (blockIdx==0 && m_isShowArrow && ROLEINFODATAHANDLER->m_pRIData->level_ <= 29)
	{
		...
		if(m_pArrow)
		{
			m_pArrow->setRotation(90.0f);
			CCNode* node = NFC_getNode(udARROWNODE,cell);
...
void BoutiqueView::_setTableCell(cocos2d::extension::CCTableViewCell* cell,int idx)
{
	idx*=2;
...
void BoutiqueView::_showBlock2(cocos2d::CCNode* cell,bool isShow){
	for (int i=udBUY2;i<=udVIEWDEBRIS2;i+=2)
		NFC_showNode(i,cell,isShow);
}

void BoutiqueView::_mapCellBtn(cocos2d::extension::CCTableViewCell* cell,int blockIdx){

	CCMenu* menu=(CCMenu*)cell->getChildByTag(udBUYBTN1+blockIdx%2);
	CMenuItemSprite* cc=(CMenuItemSprite*)menu->getChildByTag(0);
	cc->setIDX(blockIdx);
	cc->setTarget(this,menu_selector(BoutiqueView::On_Cell_SellEvent));

	DTAwardBoard* dd=(DTAwardBoard*)cell->getChildByTag(udVIEWDEBRIS1+blockIdx%2);
	dd->setIDX(blockIdx);
	dd->setTarget(this,menu_selector(BoutiqueView::On_Cell_ClickEvent));

}

void BoutiqueView::_setCellBlockInfo(cocos2d::extension::CCTableViewCell* cell,int blockIdx)
{
	ShopInfoGoodsData *data = SHOPINFODATAHANDLER->m_ShopGoodsBaselist[blockIdx];
...
void BoutiqueView::On_Cell_SellEvent(cocos2d::CCObject* pSender){
	BTN_INTERVAL("BoutiqueView_Sell",_COMMON_BTN_CLICK_INTERVAL_SMALL);
...
void BoutiqueView::On_Cell_ClickEvent(cocos2d::CCObject* pSender){
	CMenuItemSprite* cc=(CMenuItemSprite*)pSender;
	int idx  =cc->getIDX();
	ShopInfoGoodsData *data = SHOPINFODATAHANDLER->m_ShopGoodsBaselist[idx];
	_showgoodstips(data);
...
void BoutiqueView::_showgoodstips(ShopInfoGoodsData *data)
{
...
/*
	tips回调事件
*/
void BoutiqueView::cancelTipsLayer(){

}

void BoutiqueView::_enableBtnsCell(bool enabled,CCNode* cell,int blockIdx){
	_enableBtnCell(udBUYBTN1+blockIdx%2,enabled,cell);
	NFC_showNode(udBUYBTNGREY1+blockIdx%2,cell,!enabled);
}

void BoutiqueView::_enableBtnCell(int key,bool enabled,CCNode* cell){
	CCMenu* menu=(CCMenu*)cell->getChildByTag(key);
	CMenuItemSprite* cc=(CMenuItemSprite*)menu->getChildByTag(0);
	cc->setEnabled(enabled);
}

void BoutiqueView::HideWaittingLayer()
{
	//NFC_showNode(udColorLayer,this,false);
	//NFC_showNode(udANCOMMON,this,false);
}

void BoutiqueView::_setShopBuyEnableBtn(ShopInfoGoodsData *data,cocos2d::extension::CCTableViewCell* cell,int blockIdx)
{
...
void BoutiqueView::_setRedText(ShopInfoGoodsData *data,cocos2d::extension::CCTableViewCell* cell,int blockIdx)
{
	if (data->discount == -1)
	{
		//不打折
...
void BoutiqueView::_setmoneycell(ShopInfoGoodsData *data,cocos2d::extension::CCTableViewCell* cell,int blockIdx)
{
	if (data->moneyType == _TYPE_GOLD_)
	{
		//正价货币类型
...
bool BoutiqueView::onUpdateText()
{
	_setMoneyText();
	...
	{	
		...
		for (j;j<numberOfCells;j++)
		{
			...
			if (pNode!= NULL)
			{
				if (SHOPINFODATAHANDLER->m_selledshopItemId == SHOPINFODATAHANDLER->m_ShopGoodsBaselist[0]->shopItemId)
				{
					NFC_setNodeVisable(udARROWNODE,pNode,false);
					SHOPINFODATAHANDLER->m_iShowArrow = false;
				}
...
void BoutiqueView::_setrequireInfo(ShopInfoGoodsData *data,cocos2d::extension::CCTableViewCell* cell,int blockIdx)
{
...
void BoutiqueView::_setdaylimit(ShopInfoGoodsData *data,cocos2d::extension::CCTableViewCell* cell,int blockIdx)
{
	if (data->limit != -1)
	{
...
void BoutiqueView::_setMoneyText()
{
	char tmp[64] ={0};
...
void BoutiqueView::_showMoneyText()
{
