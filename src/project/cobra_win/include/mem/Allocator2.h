#ifndef  _LC_COBRA_WIN_ALLOCATOR2_H
#define  _LC_COBRA_WIN_ALLOCATOR2_H
...
template  <size_t iArraySize> 
class  Allocator2 : public  NonCopyable
{
public:
...
public:
	...
	*/
	...
	{
		size_t iPos = query_256n_index(iSize);
		if (iPos >= iArraySize)
		{
			MemNode* pNode = Malloc::new_object<MemNode>();
			pNode->init(iSize);
	...
	*/
	inline    void       insert_mem_node(MemNode* pNode)
	{
		...
		size_t iPos = query_256n_index(pNode->size());
		if (iPos >= iArraySize)
		{
